...
u8 gt1x_int_type = 1; //0:rising  1:falling
u32 gt1x_abs_x_max = 0;
u32 gt1x_abs_y_max = 0;
int gt1x_halt = 0;

#define GESTURE_CHECK_COUNT 40   //total time = 40 x 5ms
bool gt1x_dev_halt = false;
...
void gt1x_read_timeout(void)
{
    int count = 0;
    while(gt1x_dev_halt == true)
    {
	msleep(5);
	if((count++) >= GESTURE_CHECK_COUNT) {
	    GTP_ERROR("gesture read wait timeout!");
	    return;
	}
    }
}
...
static s32 gt1x_wakeup_sleep(void)
{
    ...
    while (retry++ < 2) {
#if GTP_GESTURE_WAKEUP
        if (gesture_enabled && gt1x_wakeup_gesture) {
...
s32 gt1x_request_event_handler(void)
{
 ...
 static void gt1x_report_palm(struct input_dev *input_dev)
{
	if(gt1x_palm_sleep == 0)
	{
		GTP_INFO("palm sleep is disable , return directly.\n");
		return;
	}
#if GTP_ICS_SLOT_REPORT
	input_mt_slot(input_dev, 0);
	input_mt_report_slot_state(input_dev,MT_TOOL_FINGER,true);
	input_report_abs(input_dev, ABS_MT_PRESSURE, 1000);
...
void gt1x_charger_config(s32 dir_update)
{
    ...
    if(gt1x_touch_mode != 0) {
        GTP_INFO("In smart cover mode, skip charger config.");
        return;
    }
#endif
		...
		struct device_attribute *attr, const char *buf, size_t size)
{
	u8 recv = 0;
	s32 ret = 0;
	u8 cfg_grp1_hand[] = GTP_CFG_GROUP0;
	u8 cfg_grp1_glove[] = GTP_GLOVE_CFG_GROUP0;
	u8 cfg_grp1_leather[] = GTP_SMART_COVER_CFG_GROUP0;
	u8 cfg_grp2_hand[] = GTP_CFG_GROUP1;
	u8 cfg_grp2_glove[] = GTP_GLOVE_CFG_GROUP1;
	u8 cfg_grp2_leather[] = GTP_SMART_COVER_CFG_GROUP1;

	if (kstrtou8(buf, 10, &recv))
		return -EINVAL;

	//HAND:0 GLOVE:1 LEATHER:2
	...
	} else if (gt1x_version.sensor_id == 1) {
		if (recv == 0) {
...
/**
 * gt1x_wakeup_palm_show -  palm sleep attribute show added by ztemt 2015.01.03.
 * @dev: i2c device struct.
 * @attr: device attribute.
 * @buf: data buffer.
 * Return data size that writes to the buffer.
 */
static ssize_t gt1x_palm_sleep_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	return sprintf(buf, "%d\n", gt1x_palm_sleep);
}
...
/**
 * gt1x_wakeup_gesture_store -   palm sleep attribute store added by ztemt 2015.01.03.
 * @dev: i2c device struct.
 * @attr: device attribute.
 * @buf: data buffer.
 * @size:data buffer size.
 * Return data size that writes to the buffer.
 */
static ssize_t gt1x_palm_sleep_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t size)
{
	u8 recv = 0;
	...
	gt1x_palm_sleep = recv;

	GTP_INFO("success to set palm sleep as %d", gt1x_palm_sleep);
...
static ssize_t gt1x_swap_buttons_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	return sprintf(buf, "%d\n", swap_buttons);
}

static ssize_t gt1x_swap_buttons_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t size)
{
...
static struct device_attribute gt1x_attrs[] = {
	...
	__ATTR(swap_buttons, 0664, gt1x_swap_buttons_show, gt1x_swap_buttons_store),
 ...
 */
int gt1x_create_sysfs_interfaces(struct device *dev)
{
	int i = 0;
	int ret = 0;
 ...
 * @client: i2c device struct.
 */
void gt1x_remove_sysfs_interfaces(struct device *dev)
{
	int i = 0;
	for (i = 0; i < ARRAY_SIZE(gt1x_attrs); i++)
		device_remove_file(dev, gt1x_attrs + i);
...
#endif
    gt1x_irq_disable();
    gt1x_cancle_irq_work();

    gt1x_release_fingers();
#if GTP_GESTURE_WAKEUP
