...
//Enter Gesture mode -- output report
#define WR_LENGTH_ENTER_GESMODE 1

//Enter Gesture mode -- input report
#define INPUT_BYTE_ENTER_GESMODE_EVENTCODE 5
#define INPUT_BYTE_ENTER_GESMODE_DATA 6

#define INPUT_BYTE_WAKEUP_REPORT_EVENTCODE 3
#define INPUT_BYTE_WAKEUP_REPORT_GESID 4

#define EVENT_CODE_DOUBLE_CLICK 0x01
#define EVENT_CODE_PRE_DEFINE 0x02
#define EVENT_CODE_USER_DEFINE 0x04
#define EVENT_CODE_MASK (EVENT_CODE_DOUBLE_CLICK|EVENT_CODE_PRE_DEFINE|EVENT_CODE_USER_DEFINE)

#define BIT_GES_MODE_ENTER  0x80
#define BIT_ENABLE_DOUBLE_CLICK 0x01
#define BIT_ENABLE_PRE_DEFINE 0x02
#define BIT_ENABLE_USER_DEFINE 0x04
#define ENABLE_GES_MODE_MASK (BIT_ENABLE_DOUBLE_CLICK|BIT_ENABLE_PRE_DEFINE|BIT_ENABLE_USER_DEFINE)
#endif
		...
		struct cyttsp5_hid_cmd *hid_cmd)
{
	...
	rc = cyttsp5_adap_write_read_specific(cd, cmd_length, cmd, NULL);
	if (rc)
		...
		struct cyttsp5_hid_output *hid_output)
{
	...
	if (length + 2 > CYTTSP568_PREALLOCATED_CMD_BUFFER) {
...
{
	#ifdef ZTE_GES_WAKEUP
	int write_length = WR_LENGTH_ENTER_GESMODE;
	u8 param[WR_LENGTH_ENTER_GESMODE] = { data };
	#else
	int write_length = 1;
	u8 param[1] = { data };
	#endif
	...
	#ifdef ZTE_GES_WAKEUP
	if((data & EVENT_CODE_MASK) == 0){
		dev_err(cd->dev, "%s: no event code in data\n", __func__);
	}
	#endif

	rc = cyttsp5_hid_send_output_and_wait_(cd, &hid_output);
	if (rc) {
		dev_dbg(cd->dev, "%s: enter1.\n", __func__);
		return rc;
	}

	#ifdef ZTE_GES_WAKEUP
	*return_data++ = cd->response_buf[INPUT_BYTE_ENTER_GESMODE_EVENTCODE];
/*** ZTEMT start ,can't echo easywake_gesture,20150103***/
	// *return_data = cd->response_buf[INPUT_BYTE_ENTER_GESMODE_DATA];
	*return_data = ((~cd->response_buf[INPUT_BYTE_ENTER_GESMODE_DATA])&0x07);
/*ZTEMT end*/
	#else
	*return_data = cd->response_buf[5];
	#endif

	dev_dbg(cd->dev, "%s: enter2.\n", __func__);

	return rc;
}

#ifdef ZTE_GES_WAKEUP //This is a special cmd!
static int cyttsp5_hid_send_force_exit_and_wait_(struct cyttsp5_core_data *cd,
		struct cyttsp5_hid_output *hid_output)
{
	int rc;
	int t;
...
*/
	...
	cmd[1]=0x00;
	cmd[2]=0x00;
	cmd[3]=hid_output->command_code;
	dev_vdbg(cd->dev, "%s: cyttsp5_adap_write_read_specific\n", __func__);
	rc = cyttsp5_adap_write_read_specific(cd, sizeof(cmd), cmd, NULL);//	rc = cyttsp5_adap_write_read_specific(cd, sizeof(cmd) + 2, cmd, NULL);

	if (rc){
		dev_err(cd->dev, "%s: write_read_specific return error\n",__func__);
		goto error;
	}
	...
	//cyttsp5_check_command(cd, hid_output, false);
	if(cd->response_buf[0]!=0x05){
		dev_err(cd->dev, "%s: force exit resp erro: [0]=0x%x (0x05)\n",__func__, cd->response_buf[0]);
		goto error;
	}
	if(cd->response_buf[1]!=0x00){
		dev_err(cd->dev, "%s: force exit resp erro: [1]=0x%x (0x00)\n",__func__, cd->response_buf[1]);
		goto error;
	}
	if(cd->response_buf[2]!=0xf0){
		dev_err(cd->dev, "%s: force exit resp erro: [2]=0x%x (0xf0)\n",__func__, cd->response_buf[2]);
		goto error;
	}
	if(cd->response_buf[3]!=0x00){
		dev_err(cd->dev, "%s: force exit resp erro: [3]=0x%x (0x00)\n",__func__, cd->response_buf[3]);
		goto error;
	}
	if((cd->response_buf[4]&0x0f)!=0x08){
		dev_err(cd->dev, "%s: force exit resp erro: [4]=0x%x (0xx8)\n",__func__, cd->response_buf[4]);
		goto error;
	}
	dev_dbg(cd->dev, "%s: exit success\n", __func__);
...
exit:
	...
	dev_dbg(cd->dev, "%s: exit success2.\n", __func__);
		...
		struct cyttsp5_core_data *cd, u8 data, u8 *return_data)
{
	int rc;
	struct cyttsp5_hid_output hid_output = {
		HID_OUTPUT_APP_COMMAND(0x08),    //HID_OUTPUT_APP_COMMAND(HID_OUTPUT_EXIT_EASYWAKE_STATE),
	};

	dev_dbg(cd->dev, "%s: enter\n", __func__);

	rc = cyttsp5_hid_send_force_exit_and_wait_(cd, &hid_output);
	if (rc)
		return rc;

	*return_data = cd->response_buf[3] & 0x01;

	dev_dbg(cd->dev, "%s: exit\n", __func__);
	return rc;
}
#endif
		...
		u16 *actual_read_len)
{
    ...
    if(((cd->response_buf[4] & 0x7f) == 0x24) && (cd->response_buf[6] == 0)) {
            i++;
            if(i == 1) {
                memcpy(read_buf, cd->response_buf, size);
                *actual_read_len = size;
                } else {
                memcpy(read_buf, &cd->response_buf[10], size-10);
                *actual_read_len = size-10;
                }
        ...
        if (write_len < len)
            ...
            command_code = write_buf[HID_OUTPUT_CMD_OFFSET] & HID_OUTPUT_CMD_MASK;

    /* Do not print for EXEC_PANEL_SCAN & RETRIEVE_PANEL_SCAN commands */
        if (command_code != HID_OUTPUT_EXEC_PANEL_SCAN && command_code != HID_OUTPUT_RETRIEVE_PANEL_SCAN)
...
static int cyttsp5_get_config_ver_(struct cyttsp5_core_data *cd)
{
	...
	if (rc)
        goto exit;
    dev_info(cd->dev, "%s: zte config_ver = %d \n",__func__,config_ver);

/*** ZTEMT Added by luochangyang, 2013/09/12 ***/
      si->cydata.fw_ver_conf = config_ver;
        cd->cpdata->fw_ver_ic = config_ver;

        si->update_flag = cd->cpdata->check_version(cd->cpdata, cd->dev);
        dev_info(cd->dev, "%s: FW in TP:0x%04X  FW in driver:0x%04X\n",
            __func__, cd->cpdata->fw_ver_ic, cd->cpdata->fw_ver_dr);
        dev_info(cd->dev, "%s: update_flag:%d  Firmware File Name %s\n",
            __func__,si->update_flag, cd->cpdata->cy_fw_file_name);
/***ZTEMT END***/
...
static void cyttsp5_watchdog_work(struct work_struct *work)
{
	...
	/* used for forbidding deadlock added by ztemt */
	if (watchdog_stop_flag == 1) {
		dev_err(cd->dev, "%s: just return\n", __func__);
		return;
	}
...
static int cyttsp5_put_device_into_easy_wakeup_(struct cyttsp5_core_data *cd)
{
	...
	if (rc)
		return -EBUSY;
	if ((status[0] & EVENT_CODE_MASK) == 0)
		return -EBUSY;
	if (status[1] & BIT_GES_MODE_ENTER)
		return -EBUSY;
	if ((status[1] & ENABLE_GES_MODE_MASK) == 0)
		return -EBUSY;
	#else
	if (rc || status == 0)
		return -EBUSY;
...
static int cyttsp5_wakeup_host(struct cyttsp5_core_data *cd)
{
	int rc = 0;
	int event_id;
    #ifndef ZTE_GES_WAKEUP
	int size = get_unaligned_le16(&cd->input_buf[0]);
    #endif

	/* Validate report */
	#ifdef ZTE_GES_WAKEUP
	if (cd->input_buf[2] != 4) //Current version do not check size
		rc = -EINVAL;
	#else
	...
	#ifdef ZTE_GES_WAKEUP
	event_id = cd->input_buf[INPUT_BYTE_WAKEUP_REPORT_EVENTCODE];
	gesture_id = get_unaligned_le16(&cd->input_buf[INPUT_BYTE_WAKEUP_REPORT_GESID]); //Current version do not report point information
	#else
	...
	#ifdef ZTE_GES_WAKEUP
	//current version doesn't report point
	dev_dbg(cd->dev, "%s: e=%d, gesture_id=%d, rc=%d\n", __func__, event_id, gesture_id, rc);
	#else
...
static int cyttsp5_parse_input(struct cyttsp5_core_data *cd)
{
	...
	if (report_id == HID_WAKEUP_REPORT_ID) {
        ...
        wake_lock_timeout(&cd->wake_lock, 3*HZ);
		return 0;
	}
...
static int cyttsp5_read_input(struct cyttsp5_core_data *cd)
{
	...
	if (!IS_DEEP_SLEEP_CONFIGURED(cd->easy_wakeup_gesture)) {
		if (cd->sleep_state == SS_SLEEP_ON) {
			...
			if (IS_TMO(t)){
				cyttsp5_queue_startup(cd);
                dev_err(dev, "%s: cyttsp5_queue_startup\n",__func__);
             }
			goto read;
		}
	}
...
/*** ZTEMT start 20141219 Secure touch ***/
static irqreturn_t cyttsp5_filter_interrupt(struct cyttsp5_core_data *cd);
/*** ZTEMT end ***/


static void cyttsp5_irq_work_function(struct work_struct *work)
{
	struct cyttsp5_core_data *cd =  container_of(work, struct cyttsp5_core_data, irq_work);
	int rc;
...
/*** ZTEMT start 20141219 Secure touch  ***/
	if (IRQ_HANDLED == cyttsp5_filter_interrupt(cd))
		goto end;
/*** ZTEMT end ***/
	...
	if (!rc)
		cyttsp5_parse_input(cd);
    #else
	queue_work(cd->cyttsp5_wq, &cd->irq_work);
	#endif
end:
	return IRQ_HANDLED;
}
		...
		struct cyttsp5_core_data *cd)
{
	...
	rc = cyttsp5_hid_output_exit_easywake_state_(cd,
			cd->easy_wakeup_gesture, &status);
	if (rc || status == 1){ //status = 0 ON, status = 1 SLEEP
		...
		struct device_attribute *attr, char *buf)
{
	...
	return sprintf(buf,
		...
		"%s: 0x%02X\n"
		"%s: 0x%02X\n",
		"Cyttsp5 Firmware Version in TP IC ",cd->cpdata->fw_ver_ic,
		"Cyttsp5 Firmware Version in Driver",cd->cpdata->fw_ver_dr,
		...
		struct device_attribute *attr, const char *buf, size_t size)
{
	struct cyttsp5_core_data *cd = dev_get_drvdata(dev);
	int rc;
	/*ztemt if updating firmware, forbid reset*/
	if(fw_updating)
		return size;
	/*ztemt end*/
	rc = cyttsp5_startup(cd, true);
...
static ssize_t cyttsp5_easy_sleep_palm_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	struct cyttsp5_core_data *cd = dev_get_drvdata(dev);
	ssize_t ret;
...
static ssize_t cyttsp5_easy_sleep_palm_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t size)
{
	struct cyttsp5_core_data *cd = dev_get_drvdata(dev);
	unsigned long value;
		...
		struct device_attribute *attr, char *buf)
{
	...
	for(tries=0; tries<3; tries++) {
		rc = cyttsp5_hid_output_get_param_(cd, 0xD0, &cur_mode);
		if(rc<0)
			msleep(20);
		else
			break;
	...
	dev_vdbg(dev,
			"%s: tries=%d rc=%d cur_mode=0x%02X\n",
...
/*
 * touch mode enable store via sysfs
 */
		...
		struct device_attribute *attr, const char *buf, size_t size)
{
	...
	if((value==1) || (value==3)){
		for(tries=0; tries<3; tries++) {
			rc = cyttsp5_hid_output_set_param(cd, 0xD0, value);
			if(rc < 0)
				msleep(20);
			else{
				dev_err(dev, "%s: success to set touch mode as 1 or 3 \n", __func__);
				break;
			}
		}
		if(rc < 0) {
...
static struct device_attribute attributes[] = {
	...
	__ATTR(palm_sleep, S_IRUSR | S_IWUSR,
		cyttsp5_easy_sleep_palm_show,
		cyttsp5_easy_sleep_palm_store),
...
/*** ZTEMT start 20141219 Secure touch && sleeping mode ***/

#if 0  //#if defined(CONFIG_HAS_EARLYSUSPEND) || defined(CONFIG_FB)
...
static void cyttsp5_secure_touch_notify(struct cyttsp5_core_data *cd)
{

    struct i2c_client *client = to_i2c_client(cd->dev);
	sysfs_notify(&client->dev.kobj, NULL, "secure_touch");
}

static irqreturn_t cyttsp5_filter_interrupt(struct cyttsp5_core_data *cd)
{
	if (atomic_read(&cd->st_enabled)) {
		if (atomic_cmpxchg(&cd->st_pending_irqs, 0, 1) == 0)
			cyttsp5_secure_touch_notify(cd);
		return IRQ_HANDLED;
	}
	return IRQ_NONE;
}
...
#if defined(CONFIG_SECURE_TOUCH)
static void cyttsp5_secure_touch_stop(struct cyttsp5_core_data *cd, int blocking)
{
	if (atomic_read(&cd->st_enabled)) {
		atomic_set(&cd->st_pending_irqs, -1);
        cyttsp5_secure_touch_notify(data);
		if (blocking)
			wait_for_completion_interruptible(&cd->st_powerdown);
	}
}
#else
static void cyttsp5_secure_touch_stop(struct cyttsp5_core_data *cd, int blocking)
...
#if defined(CONFIG_SECURE_TOUCH)
static void cyttsp5_secure_touch_init(struct cyttsp5_core_data *cd)
{
	init_completion(&cd->st_powerdown);
}
#else
static void cyttsp5_secure_touch_init(struct cyttsp5_core_data *cd)
{
		...
		unsigned long event, void *data)
{
	...
	dev_info(cd->dev, "fb enter\n");

/*** ZTEMT start 20141219 Secure touch && sleeping mode ***/
    if (evdata && evdata->data && cd ) {
        if (event == FB_EARLY_EVENT_BLANK){
            cyttsp5_secure_touch_stop(cd, 0);
            }
        else if (event == FB_EVENT_BLANK && evdata) {
            blank = evdata->data;
            if (*blank == FB_BLANK_UNBLANK) {
                dev_info(cd->dev, "fb UNBLANK!\n");
                cyttsp5_core_rt_resume(cd->dev);//cyttsp5_pm_runtime_get(cd);
                cyttsp5_resume_attention(cd);//remove for avoiding system halted
                dev_dbg(cd->dev, "fb resume exit\n");
            } else if (*blank == FB_BLANK_POWERDOWN) {
                dev_info(cd->dev, "fb POWERDOWN!\n");
                cyttsp5_core_rt_suspend(cd->dev);//cyttsp5_pm_runtime_put(cd);
                dev_info(cd->dev, "fb suspend exit\n");
                }
            }
        }
/*ZTEMT end*/
...
static int cyttsp5_setup_irq_gpio(struct cyttsp5_core_data *cd)
{
	...
	/* Initialize IRQ */
	cd->irq = gpio_to_irq(irq_gpio_number);
	if (cd->irq < 0)
		return -EINVAL;
		...
		u16 irq, size_t xfer_buf_size)
{
	...
	cd->bus_ops = ops;
	scnprintf(cd->core_id, 20, "%s%d", CYTTSP568_CORE_NAME, core_number++);
    ...
    cd->cyttsp5_wq = create_singlethread_workqueue("cyttsp5_wq");
	if (!cd->cyttsp5_wq) {
	    dev_err(dev, "%s: Creat workqueue failed\n", __func__);
	    rc = ENOMEM;
		goto error_create_work_queue;
	}

	/* Initialize works */
	INIT_WORK(&cd->startup_work, cyttsp5_startup_work_function);
	INIT_WORK(&cd->watchdog_work, cyttsp5_watchdog_work);
	INIT_WORK(&cd->irq_work, cyttsp5_irq_work_function);
	...
	cd->tthe_debugfs = debugfs_create_file(CYTTSP568_TTHE_TUNER_FILE_NAME,
...
#endif
/*** ZTEMT start 20141219 Secure touch ***/
    cyttsp5_secure_touch_init(cd);
    wake_lock_init(&cd->wake_lock, WAKE_LOCK_SUSPEND, "cyttsp5_wakelock");
